...
function renderBar(usage, width = 8) {
  ...
  const usedPercent = 100 - usage.remainingPercent;
  const filled = Math.round((usedPercent / 100) * safeWidth);
  return `[${'█'.repeat(filled)}${'░'.repeat(safeWidth - filled)}] ${Math.round(usedPercent)}% used`;
}
